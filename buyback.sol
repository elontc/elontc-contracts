...
contract ELONTCBuybackV1 is Ownable {
    using SafeMath for uint;

    ParentToken public token;
    address public manager;

    constructor () { }

    function buybackAndBurn(uint amount) external {
        require(
        ...
        // generate the uniswap pair path of usdc -> token
        ...
        IERC20(address(token)).transfer(address(0xdead), IERC20(address(token)).balanceOf(address(this)));
    }
